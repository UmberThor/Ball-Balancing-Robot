...
L1 = 80.0                                # length of link 1 (upper Pi -> Bi)
L2 = 80.0                                # length of link 2 (lower Mi -> Pi)
...
# solve_P: given the coordinates of the motor (M) and of the ball joint (B),
# and the scalar factor that determines on which plane the pin joint must lie on (E),
# find the coordinates of the pin joint
...
def solve(u, h):
    n = np.array([u[0], u[1], 1.0]) # from the control action (negative gradient) we can compute the normal
    n = n / np.linalg.norm(n)

    # BALL JOINTS POSITION in direct form in h and n
    B1 = np.array([
